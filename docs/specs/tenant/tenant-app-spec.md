# Tenant App Specification – Taurex.one

## 1. Authentication & Access

| Aspect | Specification |
|---|---|
| **Entry point** | https://app.taurex.one (dev: http://localhost:3001) |
| **Auth provider** | Firebase Authentication (email + password) |
| **Auth flow** | 1) User enters email/password → `signInWithEmailAndPassword()`. 2) On success, resolve `users/{uid}` → extract `tenantId`. 3) Fetch `tenants/{tenantId}` → load tenant context (including `languages`). 4) If `users/{uid}` does not exist or `tenantId` is invalid → sign out + "Not authorized" error. 5) If valid → redirect to `/`. |
| **Session persistence** | Default Firebase persistence — survives browser restarts |
| **Route guard** | `AuthGuard` wraps all authenticated routes. On mount: if `loading` → show spinner; if `!user` → redirect to `/login`. After auth, `TenantContext` resolves tenant. If resolution fails → sign out + redirect. |
| **Logout** | Button in sidebar → `signOut()` → redirect to `/login` |

### Tenant Resolution

After Firebase Auth succeeds, the app must resolve the tenant before rendering any dashboard content:

1. Read `users/{uid}` → extract `tenantId`
2. Read `tenants/{tenantId}` → load tenant metadata (including `languages` array)
3. If `languages` is empty/missing, default to `["en"]`
4. Provide `tenantId`, `languages`, and `refreshTenant()` via `TenantContext` to all pages
5. All service calls use this `tenantId` to scope Firestore paths
...
/settings                → Tenant settings: languages (autosave)
```
...
| Descriptions | Textarea per language | One textarea for each language from tenant's `languages` array. Label: "{Language} ({CODE})". |
...
One amenity list **per language** (from tenant's `languages` array).
...
**Default Price ({baseCurrency}/night)**: Number input, **mandatory**. Currency label is dynamic from tenant settings.
...
**Purpose**: Tenant configuration — base currency and language management.

### Tenant Info

Read-only display: Tenant ID, Name, Slug.
...
- Changes autosave (1s debounce) via `updateTenant(tenantId, { languages, baseCurrency })`.
- After save, `refreshTenant()` updates the context so all pages immediately see updated settings.
...
Apartments List Page
  └── fetchApartments(tenantId)
  └── deleteApartment(tenantId, slug)

Apartment Edit Page
  └── fetchApartmentBySlug(tenantId, slug)
  └── createApartment(tenantId, apartment)
  └── updateApartment(tenantId, slug, data)       ← autosave
  └── fetchSeasons(tenantId)
  └── uploadImage / uploadBigImage                 ← future

Seasons Page
  └── fetchSeasons(tenantId, year)
  └── setSeason(tenantId, seasonId, data)          ← autosave
  └── deleteSeason(tenantId, seasonId)
  └── copySeasonsToYear(tenantId, fromYear, toYear)

Settings Page
  └── updateTenant(tenantId, { languages, baseCurrency })  ← autosave
  └── refreshTenant()
```
