# Site App — Specification

## 1. Overview

The site app (`@taurex/site`) serves two audiences from a single deployment:

1. **Platform visitors** — see the Taurex.one landing page (features, pricing, CTA to sign up)
2. **Booking guests** — see a tenant's public apartment listing and individual apartment pages

Routing is path-based: `taurex.one` = landing, `taurex.one/{tenantSlug}` = tenant pages.
...
```
/                                → Landing page (Taurex platform marketing)
/{tenantSlug}                    → Tenant home (apartment listing)
/{tenantSlug}/{apartmentSlug}    → Apartment detail + booking
*                                → 404 Not Found
```
...
|---|---|
| **Available locales** | Determined by `tenant.languages` (e.g. `["en", "de"]`) |
| **Default** | First language in tenant's `languages` array (usually `en`) |
| **Locale selection** | `?lang=de` query parameter |
| **Switcher** | Language pills in tenant page header. Only shown when tenant has 2+ languages. |
| **Landing page** | English only (no switcher) |
...
The platform marketing page. Targets prospective tenants (property owners).

Already implemented. Contains:
- **Nav**: Taurex logo, Features/Pricing anchor links, Sign In + Get Started buttons → `app.taurex.one`
- **Hero**: "Vacation rentals, simplified" headline + CTA buttons
- **Features**: 4-card grid (Booking Pages, Calendar, Custom Domains, Secure & Simple)
- **Pricing**: 3-tier cards (Starter/Professional/Business)
- **CTA**: Final "Get Started" call-to-action
- **Footer**: Copyright

This page has no tenant context and no language switching.

---

## 6. Tenant Home Page (`/{tenantSlug}`)

### 6.1 Tenant Resolution

1. Extract `tenantSlug` from URL
2. Call `fetchTenantBySlug(tenantSlug)`
3. If not found → render **Tenant Not Found** page
4. If found → fetch apartments via `fetchApartments(tenantId)`
...
| **Price badge** | Overlaid on image (top-right): "{currency} {priceDefault}/night". Uses `tenant.baseCurrency`. |
| **Name** | `apartment.name` |
| **Fact pills** | Rounded badges: Guests, Bedrooms, Bathrooms, m² |
| **Link** | Entire card is clickable → `/{tenantSlug}/{apartmentSlug}` (preserves query params) |

### 6.7 Tenant Not Found

Centred message: "Tenant not found" + link back to `taurex.one`.

---

## 7. Apartment Detail Page (`/{tenantSlug}/{apartmentSlug}`)

### 7.1 Data Loading

1. Resolve tenant (same as tenant home)
2. Call `fetchApartmentBySlug(tenantId, apartmentSlug)`
3. If apartment not found → render **Apartment Not Found**
4. Fetch seasons via `fetchSeasons(tenantId)` (for min stay display)

### 7.2 Layout

**Header**: Same as tenant home (tenant name + language switcher).

**Breadcrumb**: Tenant name → Apartment name
...
- White rounded card containing:
  1. **Selected dates** — Read from `?checkIn` and `?checkOut` URL params (carried from tenant page). Dates displayed in `dd.mm.yyyy` format. If no dates, show italic hint: "Please select your dates on the apartment listing page to see pricing."
...
Below main content. Horizontal row of pill links to other apartments of the same tenant (excluding current). Each pill shows apartment name and links to `/{tenantSlug}/{aptSlug}`.

### 7.10 Apartment Not Found

"Apartment not found" + link back to `/{tenantSlug}`.
...
Leaflet/OpenStreetMap map component. Props: `apartments`, `tenantSlug`, `lang`.
...
The default date format across the entire application (site, tenant, admin apps) is **`dd.mm.yyyy`** (e.g. `17.02.2026`).
...
All prices display the tenant's `baseCurrency` symbol:
...
| **Custom domains** | Tenant resolves by hostname instead of URL slug |
...
Apartment Detail Page
  └── fetchTenantBySlug(tenantSlug)
  └── fetchApartmentBySlug(tenantId, apartmentSlug)
  └── fetchApartments(tenantId)          ← "More apartments"
  └── fetchSeasons(tenantId)             ← min stay display
```
