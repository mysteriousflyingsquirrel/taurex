...
export default function AdminApartmentEdit() {
  const { slug, tenantId: routeTenantId } = useParams<{
    slug: string;
    tenantId: string;
  }>();
  const navigate = useNavigate();
  const { tenantId, languages, baseCurrency, readonly } = useManagedTenant();
  ...
  useEffect(() => {
    if (!tenantId) return;
    fetchSeasons(tenantId).then(setSeasons);
    if (!isNew && slug) {
      fetchApartmentBySlug(tenantId, slug).then((apt) => {
        if (apt) {
          const { id: _, ...rest } = apt;
  ...
  const handleAutosave = useCallback(
    async (data: Omit<Apartment, "id">) => {
      if (!tenantId || !slug || isNew) return;
      const { slug: _slug, ...rest } = data;
      await updateApartment(tenantId, slug, rest);
    },
    [tenantId, slug, isNew]
  );
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await createApartment(tenantId, form);
      setCreated(true);
      navigate(`/tenants/${routeTenantId}/edit/apartments/${form.slug}`, {
        replace: true,
      });
    } catch (err) {
      alert(
