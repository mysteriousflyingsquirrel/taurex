...
export default function TenantForm() {
  ...
  // Load existing tenant when editing
  useEffect(() => {
    if (!tenantId) return;
    fetchTenantBySlug(tenantId).then((t) => {
      if (!t) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isEdit) {
        ...
        const existing = await fetchTenantBySlug(finalSlug);
        if (existing) {
          setError(`A tenant with slug "${finalSlug}" already exists.`);
          setSaving(false);
          return;
        }
        await createTenant({
