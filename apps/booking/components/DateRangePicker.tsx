...
export default function DateRangePicker({ checkIn, checkOut, onCheckInChange, onCheckOutChange, placeholderFrom = "dd.mm.yyyy", placeholderTo = "dd.mm.yyyy" }: DateRangePickerProps) {
  ...
  const goBack = () => {
    ...
    if (prevMonth >= nowMonth) { setBaseYear(prev.year); setBaseMonth(prev.month); }
  };

  const goForward = () => {
    const next = addMonths(baseYear, baseMonth, 1);
    setBaseYear(next.year); setBaseMonth(next.month);
  };
  ...
  const handleDayClick = (day: string) => {
    ...
    if (selecting === "checkIn") {
      ...
      if (day <= checkIn) { onCheckInChange(day); onCheckOutChange(""); setSelecting("checkOut"); }
      else { onCheckOutChange(day); setSelecting("checkIn"); setOpen(false); }
    }
  };
  ...
  const isInRange = (day: string): boolean => {
    ...
    if (checkIn && !checkOut && hoverDay && hoverDay > checkIn && selecting === "checkOut") return day >= checkIn && day <= hoverDay;
    return false;
  };
